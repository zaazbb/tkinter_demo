...
w.wm_title('Pendulum Animation Demonstration')
w.wm_iconname('pendulum')
positionWindow(w)
...
##pack $w.msg
msg = Label(w, font=font, wraplength='4i', justify=LEFT, text="This demonstration shows how Tcl/Tk can be used to carry out animations that are linked to simulations of physical systems. In the left canvas is a graphical representation of the physical system itself, a simple pendulum, and in the right canvas is a graph of the phase space of the system, which is a plot of the angle (relative to the vertical) against the angular velocity. The pendulum bob may be repositioned by clicking and dragging anywhere on the left canvas.")
msg.pack()
...
l1 = LabelFrame(p, text="Pendulum Simulation")
l2 = LabelFrame(p, text="Phase Space")
p.add(l1)
p.add(l2)
...
c = Canvas(w, width=320, height=200, background='white', bd=2, relief=SUNKEN)
...
##}
k = Canvas(w, width=320, height=200, background='white', bd=2, relief=SUNKEN)
k.create_line([160, 200, 160, 0], fill='grey75', arrow='last', tags='y_axis')
k.create_line([0, 100, 320, 100], fill='grey75', arrow='last', tags='x_axis')
for i in range(90, -1, -10):
    k.create_line([0, 0, 1, 1], smooth=True,
                  tags='graph'+str(i), fill='grey'+str(i))
...
k.create_text([0, 0], anchor='ne', text='\u03b8', tags='label_theta')
k.create_text([0, 0], anchor='ne', text='\u03b4\u03b8', tags='label_dtheta')
k.pack(in_=l2, fill=BOTH, expand=True)
...
def showPendulum(canvas, at='', x=0, y=0):
    ...
    if at == 'at' and (x != home or y != 25):
        ...
        y2 = y - 25
        length = hypot(x2, y2)
        Theta = atan2(x2, y2) * 180 / pi
    else:
        angle = Theta * pi / 180
        x = home + length * sin(angle)
        y = 25 + length * cos(angle)
    canvas.coords('rod', home, 25, x, y)
...
def showPhase(canvas):
    ...
    for i in range(0, 100, 10):
        list = points[-i: i-12]
        if len(list) >=4:
            canvas.coords('graph'+str(i), list)
...
c.bind('<Configure>', c_Configure_callback)
def k_Configure_callback(e):
    global psh, psw
    psh = e.height / 2
    psw = e.width / 2
    e.widget.coords('x_axis', [2, psh, e.width-2, psh])
    e.widget.coords('y_axis', [psw, e.height-2, psw, 2])
    e.widget.coords('label_dtheta', [psw-4, 6])
    e.widget.coords('label_theta', [e.width-6, psh+4])
k.bind('<Configure>', k_Configure_callback)
...
def recomputeAngle():
    ...
    firstDDTheta = -sin(Theta * pi/180)*scaling
    ...
    lastDDTheta = -sin(lastTheta * pi/180)*scaling
...
def repeat(w):
    ...
    showPendulum(c)
    showPhase(k)
    animationCallbacks['pendulum'] = w.after(15, lambda: repeat(w))
animationCallbacks = {}
globals().update(locals())
animationCallbacks['pendulum'] = w.after(500, lambda: repeat(w))
