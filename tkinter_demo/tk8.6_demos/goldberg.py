...
w.wm_title('Tk Goldberg (demonstration)')
w.wm_iconname('goldberg')
w.wm resizable(0, 0)
...
##pack $w.msg -side top
msg = Label(w, font='Arial 10', wraplength='4i', justify=LEFT, text="This is a\
	demonstration of just how complex you can make your animations\
	become. Click the ball to start things moving!\n\n\"Man will always\
	find a difficult means to perform a simple task\"\n - Rube Goldberg")
msg.pack(side=TOP)
...
def destroy_callback(e):
    if e.widget == w.widget.winfo_toplevel():
        del S
        del C
        del speed
...
S['cnt'] = 0
S['message'] = "\\nWelcome\\nto\\nTcl/Tk"
speed = [int(i) for i in \
             '1 10 2 20 3 50 4 80 5 100 6 150 7 200 8 300 9 400 10 500'.split()]

##set MSTART 0; set MGO 1; set MPAUSE 2; set MSSTEP 3; set MBSTEP 4; set MDONE 5
##set S(mode) $::MSTART
MSTART, MGO, MPAUSE, MSSTEP, MBSTEP, MDONE = range(6)
S['mode'] = MSTART
...
def DoDisplay(w):
    global S, C
    ctrl = ttk.Frame(w, relief='ridge', borderwidth=2, padding=5)
    screen = Frame(w, bd=2, relief='raised')
    screen.pack(side=LEFT, fill=BOTH, expand=1)
    c = Canvas(w, width=860, height=730, bg=C['bg'], highlightthickness=0)
    c.config(scrollregion=(0, 0, 1000, 1000))
    c.yview_moveto('.05')
    c.pack(in_=screen, side=TOP, fill=BOTH, expand=1)
    c.bind('<3>', lambda e: pause.invoke())
    def destroy_callback(e):
      w.after_cancel(animationCallbacks['goldberg'])
      del animationCallbacks['goldberg']
    c.bind('<Destroy>', destroy_callback)
    DoCtrlFrame(w)
    DoDetailFrame(w)
    if root._windowingsystem != 'aqua':
      show = ttk.Button(w, text='\u00bb', command=lambda: ShowCtrl(w), width=2)
    else:
      show = Button(w, text='\u00bb', command=lambda: ShowCtrl(w), width=2,
                    highlightbackground=C['bg'])
    show.place(in_=c, relx=1, rely=0, anchor='ne')
    update()
...
def DoCtrlFrame(w):
    global S
    start = ttk.Button(w, text='Start', command=lambda: DoButton(w, 0))
    pause = ttk.Checkbutton(w, text='Pause', command=lambda: DoButton(w, 1),
                          variable=S['pause'])
    step = ttk.Button(w, text='Single Step', command=lambda: DoButton(w, 2))
    bstep = ttk.Button(w, text='Big Step', command=lambda: DoButton(w, 4))
    reset = ttk.Button(w, text='Reset', command=lambda: DoButton(w, 3))
    details = ttk.Labelframe(w)
    details.tk_raise()
    S['details'] = 0
    cb = ttk.Checkbutton(details, text='Details', variable=S['details'])
    message = ttk.Labelframe(w, text='Message')
    e = ttk.Entry(message, textvariable=S['message'], justify=CENTER)
    speed = ttk.Labelframe(w, text='Speed: 0')
    scale = ttk.Scale(speed, orient='h', from=1, to=10, variable=S['speed'])
    about = ttk.Button(w, text='About', command=lambda: About(w))
    start.grid(in_=ctrl, row=0, sticky='ew')
    ctrl.grid_rowconfigure(1, minsize=10)
    pause.grid(in_=ctrl, row=2, sticky='ew')
    step.grid(in_=ctrl, stciky='ew', pady=2)
    bstep.grid(in_=ctrl, stciky='ew')
    reset.grid(in_=ctrl, stciky='ew', pady=2)
    ctrl.grid_rowconfigure(10, minsize=18)
    details.grid(in_=ctrl, row=11, sticky='ew')
    ctrl.grid_rowconfigure(11, minsize=20)
    details.configure(labelwidget=cb)
    b = ttk.Frame(details, height=1)
    b.grid()
    details.tk_raise()
    cb.tk_raise()
    ctrl.grid_rowconfigure(50, weight=1)
    S['mode'].trace_variable('w', lambda args: ActiveGUI(w))
    S['details'].trace_variable('w', lambda args: ActiveGUI(w))
    S['speed'].trace_variable('w', lambda args: ActiveGUI(w))
    message.grid(in_=ctrl, row=98, sticky='ew', pady=5)
    e.grid(sticky='nsew')
    speed.grid(in_=ctrl, row=99, sticky='ew', pady=[0, 5])
    scale.pack(fill=BOTH, expand=1)
    about.grid(in_=ctrl, row=100, sticky='ew')
    reset.bind('<3>', lambda e: S['mode'].set(-1))
    btns = addSeeDismiss(ttk.Frame(w), demo_name)
    sep = ttk.Separator(ctrl)
    sep.grid(sticky='ew', pady=4)
    i = 0
    for b in btns.winfo_children():
        if b.winfo_class() == 'TButton':
            b2 = ttk.Button(ctrl)
            b2.grid(sticky='ew')
            for b3 in b.configure():
                b3 = b3[0]
                try:
                    b2.configure(b3, b.cget(b3))
                except:
                    pass
    btns.destroy()
