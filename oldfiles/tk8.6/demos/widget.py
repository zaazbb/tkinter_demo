...
if 'defaultFont' not in root.tk.call('font', 'names'):
    if 'TkDefaultFont' in root.tk.call('font', 'names') and 'TkFixedFont' in root.tk.call('font', 'names'):
        ...
        varsFont = Font(family='Helvetica', size=14)
...
##    2tICU0gXBQA7
##}
img_refresh = PhotoImage(format='GIF', data="""
    R0lGODlhEAAQAJEDAP///wAAACpnKv///yH5BAEAAAMALAAAAAAQABAAAAI63IKp
    xgcPH2ouwgBCw1HIxHCQ4F3hSJKmwZXqWrmWxj7lKJ2dndcon9EBUq+gz3brVXAR
    2tICU0gXBQA7
""")
...
##}
img_view = PhotoImage(format='GIF', data="""
    R0lGODlhEAAQAKIHAP///wwMDAAAAMDAwNnZ2SYmJmZmZv///yH5BAEAAAcALAAA
    AAAQABAAAANMKLos90+ASamDRxJCgw9YVnlDOXiQBgRDBRgHKE6sW8QR3doPKK27
    yg33q/GIOhdg6OsEJzeZykiBSUcs06e56Xx6np8ScIkFGuhQAgA7
""")
...
##}
img_delete = PhotoImage(format='GIF', data="""
    R0lGODlhEAAQAIABAIQAAP///yH5BAEAAAEALAAAAAAQABAAAAIjjI+pmwAc3HGy
    PUSvqYpuvWQg40FfSVacBa5nN6JYDI3mzRQAOw==
""")
...
##    ryhH5pgnEQA7
##}
img_print = PhotoImage(format='GIF', data="""
    R0lGODlhEAAQALMKAAAAAP///52VunNkl8C82Yl+qldBgq+pyrOzs1fYAP///wAA
    AAAAAAAAAAAAAAAAACH5BAEAAAoALAAAAAAQABAAAARGUMlJKwU4AztB+ODGeUiJ
    fGLlgeEYmGWQXmx7aXgmAUTv/74N4EAsGhOJg1DAbDqbwoJ0Sp0KB9isNis0eL/g
    ryhH5pgnEQA7
...
##}]
img_new = PhotoImage(format='GIF', data="""
    
...
##}
def tkAboutDialog():
    messagebox.showinfo(title='About Widget Demo',
                        message='Tk widget demonstration application',
                        detail="""
Copyright \u00a9 1996-1997 Sun Microsystems, Inc.
Copyright \u00a9 1997-2000 Ajuba Solutions, Inc.
Copyright \u00a9 2001-2009 Donal K. Fellows
Copyright \u00a9 2002-2007 Daniel A. Steffen
""".strip())
if root._windowingsystem != 'aqua':
    file = Menu(menuBar, tearoff=0)
    file.add('command', label='About...', underline=0,
             command=tkAboutDialog, accelerator='<F1>')
    root.bind('<F1>', tkAboutDialog)
    file.add('sep')
    if root._windowingsystem.startswith('win'):
        file.add('command', label='Quit', underline=0,
                 command=root.destroy, accelerator='Ctrl+Q')
        root.bind('Control-q', root.destroy)
    else:
        file.add('command', label='Quit', underline=0,
                 command=root.destroy, accelerator='Meta-Q')
        root.bind('Meta-q', root.destroy)
    menuBar.add('cascade', label='File', underline=0, menu=file)
...
statusBar_lab = ttk.Label(statusBar, text='   ', anchor='w')
if root._windowingsystem == 'aqua':
    sep = ttk.Separator(statusBar)
    sep.pack(side='top', expand='yes', fill='x', pady=0)
...
##s = Scrollbar(textFrame, orient= 'vertical', takefocus=1)
##t = Text(textFrame, wrap='word', width=70,
##         height=textheight, font=mainFont, setgrid=1, highlightthickness=0,
##         padx=4, pady=2, takefocus=0)
##s['command'] = t.yview
##t['yscrollcommand'] = s.set
##s.pack(side='right', fill='y')
##t.pack(expand='y', fill='both', padx=1)
t = ScrolledText(textFrame, wrap='word', width=70, height=textheight,
                 font=mainFont, setgrid=1, highlightthickness=0,
                 padx=4, pady=2, takefocus=0)
t.pack(expand='y', fill='both', padx=1)
...
##}
t.tag_configure('title', font=titleFont)
t.tag_configure('subtitle', font=titleFont)
t.tag_configure('bold', font=boldFont)
if root._windowingsystem == 'aqua':
    t.tag_configure('title', spacing1=8)
    t.tag_configure('subtitle', spacing1=3)
...
t.tag_configure('demospace', lmargin1='1c', lmargin2='1c')
...
##    showStatus [.t index {@%x,%y}]
##}
ttk_cursor = {'text':'ibeam', 'link':'hand2', 'busy':'wait'}
demo_name = {}
def invoke(index):
    global tk_demoDirectory
    tags = t.tag_names(index)
    demo_tags = [i for i in tags if i.startswith('demo-')]
    if len(demo_tags) == 0:
        return
    cursor = t.cget('cursor')
    t['cursor'] = ttk_cursor['busy']
    root.update()
    demo = demo_tags[0][5:]
    ...
    t.tag_add('visited', '%s linestart +1 chars'%index,
              '%s lineend -1 chars'%index)
if root.winfo_depth() == 1:
    t.tag_configure('demo', lmargin1='1c', lmargin2='1c', underline=1)
    t.tag_configure('visited', lmargin1='1c', lmargin2='1c', underline=1)
    t.tag_configure('hot', background='black', foreground='white')
else:
    t.tag_configure('demo', lmargin1='1c', lmargin2='1c',
                    foreground='blue', underline=1)
    t.tag_configure('visited', lmargin1='1c', lmargin2='1c',
                    foreground='#303080', underline=1)
    t.tag_configure('hot', foreground='red', underline=1)
t.tag_bind('demo', '<ButtonRelease-1>',
           lambda e: invoke(t.index('@%r,%r'%(e.x, e.y))))
def showStatus(index):
    ...
    if len(demo_tags) == 0:
        statusBar_lab['text'] = ''
        newcursor = ttk_cursor['text']
    else:
        demo = demo_tags[0][5:]
        statusBar_lab['text'] = 'Run the "%s" sample program'%demo
        newcursor = ttk_cursor['link'] 
    if cursor != newcursor:
        t['cursor'] = newcursor
lastLine = ''
def t_tag_bind_demo_enter(e):
    global lastLine
    lastLine = t.index('@%r,%r linestart'%(e.x, e.y))
    t.tag_add('hot', '%s +1 chars'%lastLine, '%s lineend -1 chars'%lastLine)
    t['cursor'] = ttk_cursor['link']
    showStatus(t.index('@%r,%r'%(e.x, e.y)))
t.tag_bind('demo', '<Enter>', t_tag_bind_demo_enter)
def t_tag_bind_demo_leave(e):
    t.tag_remove('hot', 1.0, 'end')
    t['cursor'] = ttk_cursor['text'] 
    statusBar_lab['text'] = ''
t.tag_bind('demo', '<Leave>', t_tag_bind_demo_leave)
##.t tag bind demo <Motion> {
##    set newLine [.t index {@%x,%y linestart}]
##    if {$newLine ne $lastLine} {
##	.t tag remove hot 1.0 end
...
##    showStatus [.t index {@%x,%y}]
##}
def t_tag_bind_demo_motion(e):
    newLine = t.index('@%r,%r linestart'%(e.x, e.y))
    global lastLine
    if newLine != lastLine:
        t.tag_remove('hot', 1.0, 'end')
        lastLine = newLine
        tags = t.tag_names('@%r,%r'%(e.x, e.y))
        demo_tags = [i for i in tags if i.startswith('demo-')]
        if len(demo_tags) > 0:
            t.tag_add('hot', '%s +1 chars'%lastLine, '%s lineend -1 chars'%lastLine)
    showStatus(t.index('@%r,%r'%(e.x, e.y)))
t.tag_bind('demo', '<Motion>', t_tag_bind_demo_motion)
...
##    }
##}
def addFormattedText(formattedText):
    style = 'normal'
    isNL = 1
    demoCount = 0
    new = 0
        ...
        if line.startswith('@@'):
            ...
            if key == 'title':
                ...
                if len(data_list) > 1:
                    ...
                    description = ''
                demoCount += 1
                t.insert('end', "%i. %s"%(demoCount, description),
                         'demo demo-%s'%name)
                if new:
                    t.image_create('end', image=img_new, padx=5)
                    new = 0
                t.insert('end', ' \n ', 'demospace')
            elif key == 'new':
                new = 1
...
def showVars(dlg, args):
    ...
    b = ttk.Frame(w)
    b.grid(sticky='news')
    f = ttk.Labelframe(b, text='Variable values:')
    for i, var in enumerate(args):
        n = ttk.Label(f, text='%s:'%(var._name), anchor='w')
        v = ttk.Label(f, textvariable=var, anchor='w')
        n.grid(column=0, row=i, padx=2, pady=2, sticky='w')
        v.grid(column=1, row=i, padx=2, pady=2, sticky='w')
    ok = ttk.Button(b, text='OK', command=w.destroy, default='active')
    w.bind('<Return>', ok.invoke)
    w.bind('<Escape>', ok.invoke)

    f.grid(sticky='news', padx=4)
    ok.grid(sticky='e', padx=4, pady='6 4')
    if root._windowingsystem == 'aqua': pass
...
def addSeeDismiss(w, show, vars_=[], extra=''):
    ...
    dismiss = ttk.Button(w, text='Dismiss', image=img_delete, compound='left',
                         command=w.winfo_toplevel().destroy)
    code = ttk.Button(w, text='See Code', image=img_view, compound='left',
                      command=lambda:
