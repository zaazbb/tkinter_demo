...
w.wm_title('Listbox Demonstration (colors)')
w.wm_iconname('Listbox')
positionWindow(w)
...
##pack $w.msg -side top
msg = Label(w, font=font_, wraplength='4i', justify='left', text="A listbox containing several color names is displayed below, along with a scrollbar.  You can scan the list either using the scrollbar or by dragging in the listbox window with button 2 pressed.  If you double-click button 1 on a color, then the application's color palette will be set to match that color")
msg.pack(side='top')
...
list_ = Listbox(frame, yscroll=scroll.set, width=20, height=16, setgrid=1)
...
list_.bind('<Double-1>', lambda e: root.tk_setPalette(list_.get(list_.curselection())))
list_.insert(0, *'gray60 gray70 gray80 gray85 gray90 gray95 \
    snow1 snow2 snow3 snow4 seashell1 seashell2 \
    seashell3 seashell4 AntiqueWhite1 AntiqueWhite2 AntiqueWhite3 \
    AntiqueWhite4 bisque1 bisque2 bisque3 bisque4 PeachPuff1 \
    PeachPuff2 PeachPuff3 PeachPuff4 NavajoWhite1 NavajoWhite2 \
